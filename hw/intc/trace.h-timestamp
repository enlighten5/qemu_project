...
static inline void _nocheck__trace_nvic_set_pending(int irq, bool secure, bool derived, int en, int prio)
{
    if (trace_event_get_state(TRACE_NVIC_SET_PENDING)) {
        ...
        gettimeofday(&_now, NULL);
        qemu_log_mask(LOG_TRACE,
                      "%d@%zd.%06zd:nvic_set_pending " "NVIC set pending irq %d secure-bank %d derived %d (enabled: %d priority %d)" "\n",
                      getpid(),
                      (size_t)_now.tv_sec, (size_t)_now.tv_usec
                      , irq, secure, derived, en, prio);
    }
}
...
static inline void _nocheck__trace_nvic_get_pending_irq_info(int irq, bool secure)
{
    if (trace_event_get_state(TRACE_NVIC_GET_PENDING_IRQ_INFO)) {
        ...
        gettimeofday(&_now, NULL);
        qemu_log_mask(LOG_TRACE,
                      "%d@%zd.%06zd:nvic_get_pending_irq_info " "NVIC next IRQ %d: targets_secure: %d" "\n",
                      getpid(),
                      (size_t)_now.tv_sec, (size_t)_now.tv_usec
                      , irq, secure);
    }
