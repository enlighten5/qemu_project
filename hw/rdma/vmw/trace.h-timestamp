...
{
    if (trace_event_get_state(TRACE_PVRDMA_REGS_READ)) {
        struct timeval _now;
        gettimeofday(&_now, NULL);
        qemu_log_mask(LOG_TRACE,
                      "%d@%zd.%06zd:pvrdma_regs_read " "regs[0x%"PRIx64"] = 0x%"PRIx64 "\n",
                      getpid(),
...
{
    if (trace_event_get_state(TRACE_PVRDMA_REGS_WRITE)) {
        struct timeval _now;
        gettimeofday(&_now, NULL);
        qemu_log_mask(LOG_TRACE,
                      "%d@%zd.%06zd:pvrdma_regs_write " "regs[0x%"PRIx64"] = 0x%"PRIx64 "\n",
                      getpid(),
                      (size_t)_now.tv_sec, (size_t)_now.tv_usec
                      , addr, val);
    }
