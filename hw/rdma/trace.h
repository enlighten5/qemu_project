...
{
    if (trace_event_get_state(TRACE_CREATE_AH_CACHE_HIT)) {
        struct timeval _now;
        gettimeofday(&_now, NULL);
        qemu_log_mask(LOG_TRACE,
                      "%d@%zd.%06zd:create_ah_cache_hit " "subnet = 0x%"PRIx64" net_id = 0x%"PRIx64 "\n",
                      getpid(),
...
{
    if (trace_event_get_state(TRACE_CREATE_AH_CACHE_MISS)) {
        struct timeval _now;
        gettimeofday(&_now, NULL);
        qemu_log_mask(LOG_TRACE,
                      "%d@%zd.%06zd:create_ah_cache_miss " "subnet = 0x%"PRIx64" net_id = 0x%"PRIx64 "\n",
                      getpid(),
                      (size_t)_now.tv_sec, (size_t)_now.tv_usec
                      , subnet, net_id);
    }
